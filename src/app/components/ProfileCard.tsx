  ...
  return (
    <div className="pt-8 flex justify-center">
      <div className="max-w-4xl bg-white rounded shadow w-full">
        ...
        {/* Add Funds Tab */}
        {selectedTab === "add_funds" && <AddFunds publicKey={publicKey} />}

        {/* Warning for unsupported tabs */}
        {(selectedTab === "send" || selectedTab === "withdraw") && <Warning />}
      </div>
    </div>
  );
};
...
function Greeting({ image, name }: { image: string | null; name: string }) {
  const fallbackImage = "/default-avatar.png"; // put a default image in public folder
