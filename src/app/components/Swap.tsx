...
function SwapInputRow({
    ...
    return (
        <div
            className={`border flex justify-between p-6 ${topBorderEnabled ? "rounded-t-xl" : ""} ${bottomBorderEnabled ? "rounded-b-xl" : ""}`}
        >
            <div>
            ...
            <div>
                <input
                    ...
                    dir="rtl"
                    value={inputLoading ? "Loading" : amount || ""}  // Default to an empty string if amount is undefined
                />
            </div>
        </div>
    );
    ...
    return (
        <div className="w-24">
            <select
                ...
                value={selectedToken.name}  // Use value instead of selected on option
                id="countries"
